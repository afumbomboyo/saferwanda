      ...
      <main className="flex-grow pt-32 pb-24">
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="text-center mb-10 animate-fade-in">
            ...
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-headline font-extrabold mb-4">SafeRwanda AI Concierge</h1>
          ...
          <Card className="bg-card border-border shadow-2xl mb-8">
            <CardHeader>
              ...
              <CardDescription className="text-sm">
                Ask anything like "I'm worried about my house when I'm at work" or "My parents live alone."
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="flex gap-2">
                <Input 
                  ...
                  className="bg-background border-border h-11 md:h-12"
                  disabled={loading}
                />
                <Button 
                  type="submit" 
                  disabled={loading || !query.trim()}
                  className="bg-primary hover:bg-primary/90 h-11 md:h-12 px-6"
                >
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {response.recommendedServices.map((service, idx) => (
                  <Card key={idx} className="bg-card border-border flex items-center p-4 gap-4 shadow-sm">
                    <div className="p-2 rounded-lg bg-primary/10">
                      <CheckCircle2 className="w-5 h-5 md:w-6 md:h-6 text-primary" />
                    </div>
                    <div>
                      <h4 className="text-sm md:text-base font-headline font-bold">{service}</h4>
